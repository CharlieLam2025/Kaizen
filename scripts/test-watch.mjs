...
assert.equal(pickTranslateRows({ 1: "甲", 2: "乙" }, 2).join("|"), "甲|乙", "模型从 1 数行时整批要对齐回来");
assert.equal(pickTranslateRows({ 1: "乙" }, 3).join("|"), "|乙|", "稀疏的 0 起始键不能被当成 1 起始");
...
assert.match(content, /VB_CONTENT_REV = 40/, "重载后要升 rev，才能重装 K");
...
assert.match(content, /45000/, "后台心跳 15 秒一跳，45 秒没跳就当不忙");
assert.match(bg, /aiPulse = setInterval\(\(\) => \{\s*chrome\.storage\.local\.set/, "保活心跳必须真调扩展 API，空函数撑不住 SW");
assert.match(bg, /^chrome\.storage\.local\.set\(\{ vb_ai_busy: 0 \}\)/m, "SW 重启要清掉残留的忙标记");
assert.match(bg, /const tries = \[raw, blob/, "弯引号是合法 JSON 字符，要先试原文再软化");
assert.match(bg, /TimeoutError/, "AbortSignal 超时也要算网络类错误再抛");
assert.match(content, /liveZhSnoozeUntil/, "busy 跳过要设冷却，不能空转热循环");
assert.match(content, /Date\.now\(\) >= liveZhSnoozeUntil/, "冷却期间片上不要再进对照");
assert.match(content, /function liveTrackChanged/, "翻译回来要先确认还是这支视频这条轨");
assert.match(content, /liveSegFinger\(pack\.segments\) !== liveSegFinger\(liveSegments\)/, "轨不对齐时译文不能写进缓存");
assert.match(panel, /translateAll\.busy === lock/, "旧翻译循环醒来不能清掉新循环的锁");
assert.match(panel, /clearTimeout\(saveCacheTimer\);\s*saveCacheTimer = 0;/, "切片要清掉上一支的缓存定时器");
assert.match(bg, /if \(!opts\?\.queue\) return callAiOnce/, "查词问老师不能排在翻译队伍后面干等");
assert.match(bg, /queue: true,\s*system: translateSystem/, "翻译批次要走串行道，别挤爆 DeepSeek");
assert.match(bg, /conceptMap\?\.propositions/, "导出笔记要读真实的命题字段，不是 edges");
assert.match(content, /__vbLoopTick/, "循环监听要走中继，扩展重装后 A 键还能用");
assert.match(content, /nextAt < liveTrackAt/, "没有时间戳的旧缓存包不能反超新轨");
assert.match(panel, /droppedRuns >= 5/, "后台一直不回应要停下来提示，不能永远转圈");
assert.match(panel, /queueBackgroundWork\(state\.videoId\)/, "切片打断拆块后新视频要重新排队");
assert.match(panel, /function pauseFollowFromScroll/, "拖滚动条和 PageDown 也要能暂停跟随");
assert.match(panel, /userIntent && \(programmaticScroll/, "滚轮不可能是程序滚动，锁窗内也不能吞");
assert.match(panelCss, /#transcriptBox\.free-read \.t-row \{ overflow-anchor: auto/, "停跟随读字幕时补翻不能推着正文跑");
assert.match(panel, /classList\.toggle\("free-read"/, "跟随状态变了要同步锚定开关");
assert.match(bg, /promise\.catch\(\(\) => \{\}\)/, "硬超时的输家请求要有人接住，不能刷红");
assert.match(content, /liveZhErrText/, "钥匙额度的真实错误要留在条子上，不能一闪而过");
assert.match(content, /const paintLex = /, "词卡回调要按 id 现取节点，不能写进拆掉的旧卡");
assert.match(panel, /data-glabel="\$\{escAttr\(/, "概念图标签进属性要用 escAttr，esc 挡不住引号");
assert.match(panel, /data-word="\$\{escAttr\(k\.word\)\}"/, "学习包关键词进属性要用 escAttr");
assert.doesNotMatch(panel, /value="\$\{esc\(/, "输入框回填要用 escAttr，esc 会被引号打穿");
assert.match(content, /title="\$\{title\}"/, "进度条章节标题要走 escLive，不能靠删引号");
assert.doesNotMatch(content, /replace\(\/"\/g, ""\)/, "书签提示不能靠删字符黑名单防注入");
assert.match(content, /\.replace\(\/>\/g, "&gt;"\)/, "escLive 要补上 > 的转义");
assert.match(panel, /replace\(\/'\/g, "&#39;"\)/, "escAttr 要补上单引号的转义");
